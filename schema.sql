...
CREATE TABLE IF NOT EXISTS fsbf_carta (
    ...
    chefe_nome            TEXT,
    chefe_contacto        TEXT,
    chefe_guarnicao       INT         CHECK (chefe_guarnicao IS NULL OR chefe_guarnicao BETWEEN 0 AND 9),
...
CREATE TABLE IF NOT EXISTS fsbf_bsbf_equipa (
    id          UUID        PRIMARY KEY DEFAULT gen_random_uuid(),
    data        DATE        NOT NULL,
    brigada     TEXT        NOT NULL CHECK (brigada IN ('Norte','Sul','GSBF','Outros')),
    base        TEXT,
    veiculo_id  UUID        REFERENCES viaturas(id) ON DELETE SET NULL,
    guarnicao   INT         CHECK (guarnicao IS NULL OR guarnicao BETWEEN 0 AND 9),
    chefe_nome  TEXT,
    contacto    TEXT,
    observacoes TEXT,
    ordem       INT         DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS fsbf_emr_equipa (
    ...
    chefe_nome         TEXT,
    contacto           TEXT,
    total_op           INT         CHECK (total_op IS NULL OR total_op BETWEEN 0 AND 9),
    observacoes        TEXT,
    ordem              INT         DEFAULT 0,
    created_by         UUID        REFERENCES utilizadores(id) ON DELETE SET NULL,
    created_at         TIMESTAMPTZ DEFAULT now(),
